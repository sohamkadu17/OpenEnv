---
title: Kube Sre Gym Environment Server
emoji: 🎧
colorFrom: blue
colorTo: pink
sdk: docker
...
The simplest way to use the Kube Sre Gym environment is through the `KubeSreGymEnv` class:
...
try:
    ...
    kube_sre_gymenv = KubeSreGymEnv.from_docker_image("kube_sre_gym-env:latest")
    ...
    for msg in messages:
        result = kube_sre_gymenv.step(KubeSreGymAction(message=msg))
        print(f"Sent: '{msg}'")
    ...
    # Always clean up
    kube_sre_gymenv.close()
```

That's it! The `KubeSreGymEnv.from_docker_image()` method handles:
...
### Action
**KubeSreGymAction**: Contains a single field
- `message` (str) - The message to echo back

### Observation
**KubeSreGymObservation**: Contains the echo response and metadata
...
If you already have a Kube Sre Gym environment server running, you can connect directly:

```python
from kube_sre_gym import KubeSreGymEnv

# Connect to existing server
kube_sre_gymenv = KubeSreGymEnv(base_url="<ENV_HTTP_URL_HERE>")
